      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur-sm text-center">
        <CardContent className="pt-8 pb-8 space-y-4">
          ...
          <p className="text-sm text-muted-foreground max-w-sm mx-auto">
            You already submitted a payment for the <strong className="capitalize">{existing.plan}</strong> plan. It is being reviewed.
          </p>
          <p className="text-xs text-zinc-500">Ref: {existing.transaction_ref}</p>
    ...
    return (
      <Card className="border-border/50 bg-card/50 backdrop-blur-sm text-center">
        <CardContent className="pt-8 pb-8 space-y-4">
          ...
          <p className="text-sm text-muted-foreground max-w-sm mx-auto">
            Your payment will be reviewed within 24 hours. You will be notified once approved.
          </p>
          <Button onClick={() => router.push("/membership")} variant="outline" className="gap-2">
            <ArrowLeft className="h-4 w-4" /> Back to Plans
          </Button>
            ...
            <div className="flex gap-2">
              {(["telebirr", "cbe"] as const).map((m) => (
                <button key={m} type="button" onClick={() => setMethod(m)}
                  className={`flex-1 p-3 rounded-xl border text-sm font-medium capitalize transition-colors ${
                    method === m ? "border-gold bg-gold/10 text-gold" : "border-zinc-800 text-muted-foreground hover:border-zinc-600"
                  }`}
                >
  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <Suspense fallback={
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm"><CardContent className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-gold" /></CardContent></Card>
        }>
          <CheckoutContent />
        </Suspense>
      </motion.div>
