...
type Submission = {
  ...
  payment_method: string; transaction_ref: string; receipt_data: string | null;
            ...
            <div key={s.id} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1.5 text-sm flex-1">
                  ...
                  {s.receipt_data && <img src={s.receipt_data} alt="Receipt" className="max-h-48 rounded-lg border border-zinc-700 mt-2" />}
                </div>
            ...
            {reviewed.map((s) => (
              <div key={s.id} className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-3 text-sm">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium">{s.profiles?.full_name || s.profiles?.email || "Unknown"}</p>
                    <p className="text-zinc-500 text-xs capitalize">{s.plan} — ${s.amount.toFixed(2)} — {s.transaction_ref}</p>
                  </div>
